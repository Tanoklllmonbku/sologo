...
fun LostReportScreen(
    ...
    LaunchedEffect(Unit) { lostViewModel.loadMyReports() }
        ...
        Column(
            ...
            Card(
                ...
                Column(modifier = Modifier.padding(16.dp)) {
                    ...
                    Text("Если вы потерялись или нуждаетесь в помощи, отправьте свои координаты. Администратор получит уведомление.", style = MaterialTheme.typography.bodyMedium)
                    Spacer(modifier = Modifier.height(16.dp))
                    Button(
                        onClick = { getLocation() }, modifier = Modifier.fillMaxWidth(),
                    ...
                    if (latitude != 0.0 && longitude != 0.0) {
                        Spacer(modifier = Modifier.height(8.dp))
                        Text("Координаты: $latitude, $longitude", style = MaterialTheme.typography.bodySmall, color = SoloGreen)
                    }
                    Spacer(modifier = Modifier.height(16.dp))
                    OutlinedTextField(
                    ...
                    Spacer(modifier = Modifier.height(16.dp))
                    Button(
                        onClick = { if (latitude != 0.0 && longitude != 0.0) lostViewModel.reportLost(latitude, longitude, message.takeIf { it.isNotBlank() }) },
                        modifier = Modifier.fillMaxWidth(), enabled = latitude != 0.0 && longitude != 0.0,
                        colors = ButtonDefaults.buttonColors(containerColor = if (latitude != 0.0 && longitude != 0.0) SoloGreen else MaterialTheme.colorScheme.surfaceVariant),
